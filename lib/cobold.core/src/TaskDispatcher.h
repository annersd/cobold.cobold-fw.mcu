...
#pragma mark - Dispatch Class -
	...
	public:
		TaskDispatcher(std::string name, size_t thread_cnt = 1, size_t thread_stack = 1024);
		~TaskDispatcher();
		...
		// Deleted operations
		TaskDispatcher(const TaskDispatcher &rhs) = delete;
		TaskDispatcher &operator=(const TaskDispatcher &rhs) = delete;
		TaskDispatcher(TaskDispatcher &&rhs) = delete;
		TaskDispatcher &operator=(TaskDispatcher &&rhs) = delete;
...
#pragma mark - Implementation -
	...
	TaskDispatcher::TaskDispatcher(std::string name, size_t thread_cnt, size_t thread_stack_size) : name_{std::move(name)}, threads_(thread_cnt)
	{
		// Create the Mutex
		...
		for (size_t i = 0; i < threads_.size(); i++)
		{
			...
			BaseType_t status = xTaskCreate(
				reinterpret_cast<void (*)(void *)>(BOUNCE(TaskDispatcher, dispatch_thread_handler)),
	...
	void TaskDispatcher::dispatch(const fp_t &op)
	{
	...
	void TaskDispatcher::dispatch(fp_t &&op)
	{
		BaseType_t status = xSemaphoreTakeRecursive(mutex_, portMAX_DELAY);
		assert(status == pdTRUE && "Failed to lock mutex!");
	...
	void TaskDispatcher::dispatch_thread_handler(void)
	{
		BaseType_t status = xSemaphoreTakeRecursive(mutex_, portMAX_DELAY);
		assert(status == pdTRUE && "Failed to lock mutex!");
